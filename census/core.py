        ...
        def wrapper(self, *args, **kwargs):
            ...
            if int(year) not in _years:
                raise UnsupportedYearException(
                    "Geography is not available in {}. Available years include {}".format(
                        year, _years
                    )
                )
...
class Client(object):
    ...
    def get(self, fields, geo, year=None, **kwargs):
        ...
        all_results = (
            self.query(
                forty_nine_fields, geo, year, sort_by_geoid=sort_by_geoid, **kwargs
            )
    ...
    @supported_years()
    def state_congressional_district(
        self, fields, state_fips, congressional_district, **kwargs
    ):
        return self.get(
            fields,
            geo={
    ...
    @supported_years()
    def state_legislative_district_upper(
        self, fields, state_fips, legislative_district, **kwargs
    ):
        return self.get(
            fields,
            geo={
                "for": "state legislative district (upper chamber):{}".format(
                    str(legislative_district).zfill(3)
                ),
                "in": "state:{}".format(state_fips),
            },
            **kwargs
    ...
    @supported_years()
    def state_legislative_district_lower(
        self, fields, state_fips, legislative_district, **kwargs
    ):
        return self.get(
            fields,
            geo={
                "for": "state legislative district (lower chamber):{}".format(
                    str(legislative_district).zfill(3)
                ),
                "in": "state:{}".format(state_fips),
            },
    ...
    def _switch_endpoints(self, year):
        if year > 2009:
            ...
            self.definitions_url = (
                "https://api.census.gov/data/%s/acs/%s/variables.json"
            )
            self.definition_url = (
                "https://api.census.gov/data/%s/acs/%s/variables/%s.json"
            )
...
class ACS5Client(ACSClient):
    ...
    @supported_years()
    def state_county_subdivision(
        self, fields, state_fips, county_fips, subdiv_fips, **kwargs
    ):
        return self.get(
            fields,
            geo={
    ...
    def zipcode(self, fields, zcta, **kwargs):
        warnings.warn(
            "zipcode has been deprecated; use state_zipcode instead", DeprecationWarning
        )
    ...
    def state_district_place(self, fields, state_fips, district, place, **kwargs):
        return self.get(
            ...
            geo={
                ...
                "in": "state:{} congressional district:{}".format(state_fips, district),
            },
            **kwargs
        )
...
class Census(object):
    ...
    def __init__(self, key, year=None, session=None):
        ...
        self.session.headers.update(
            {
                "User-Agent": (
                    "python-census/{} ".format(__version__)
